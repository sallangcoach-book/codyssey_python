...
| 번호 | 기능 | 설명 |
|---|---|---|
| ⭐ | 랜덤 출제 | `random.shuffle()`로 매번 문제 순서를 섞어서 출제합니다. |
| ⭐ | 문제 수 선택 | 몇 문제를 풀지 사용자가 직접 정할 수 있습니다. |
| ⭐ | 힌트 기능 | 문제 풀이 중 `0`을 입력하면 힌트를 보여주고, 사용 시 1점을 차감합니다. |
| 5 | 퀴즈 삭제 | 번호를 선택해 등록된 퀴즈를 삭제합니다. |
| ⭐ | 점수 기록 히스토리 | 매 플레이의 날짜/시간, 문제 수, 점수를 저장하고, 점수 확인 메뉴에서 최근 기록을 보여줍니다. |
